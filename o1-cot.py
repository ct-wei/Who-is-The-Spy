...
client = OpenAI(
    api_key=
    "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",
    )
...
def generate_response(prompt, step_container, final_container, progress_bar):
    ...
    messages = [{
        ...
        "role": "assistant",
        "content": "谢谢！我将按照指示逐步思考，从分解问题的开始。"
    }]
    ...
    while True:
        ...
        # 发送请求
        step_data = make_api_call(messages=messages,
                                  max_tokens=512,
                                  temperature=0.3)
        end_time = time.time()
        ...
        with step_container.container():
            with st.expander(f"步骤 {step_count}: {step_info['title']}",
                             expanded=True):
        ...
        # 检查是否达到最终答案
        if step_data.get(
                'next_action') == 'final_answer' or step_count >= max_steps:
            break
    ...
    start_time = time.time()
    final_data = make_api_call(messages=messages,
                               max_tokens=512,
                               temperature=0.3)
    end_time = time.time()
    thinking_time = end_time - start_time
    ...
    if st.button("生成推理链"):
        if not prompt.strip():
            ...
            with st.spinner("正在生成推理链，请稍候..."):
                generate_response(prompt, step_container, final_container,
                                  progress_bar)
